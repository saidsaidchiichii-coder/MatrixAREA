...
def mirror_self(source_dir: Path) -> str:
    ...
    Mirrors both backend/ and frontend/ so the agent can study and evolve its
    own logic AND its own UI (Stage 4: UI evolution)."""
    root = source_dir.parent if source_dir.name == "backend" else source_dir
    ...
    for sub in ("backend", "frontend"):
        src = root / sub
        if src.exists():
            shutil.copytree(src, SOURCE_MIRROR / sub)
            copied += sum(1 for p in (SOURCE_MIRROR / sub).rglob("*") if p.is_file())
    return f"mirrored {copied} source files (backend+frontend) into sandbox/self_source"
